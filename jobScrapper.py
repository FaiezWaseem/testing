...
class LinkedInJobScraper:
    def __init__(self, li_at_cookie, location="Germany", limit = 20):
    ...
    def run(self, type_filters, on_site_or_remote_filters, experience_filters, base_salary_filter):
        ...
        for job in jobs_data:
            ...
            # Write the updated job to a log file
            with open('job_log.txt', 'a', encoding='utf-8') as log_file:
                log_file.write(json.dumps(job, ensure_ascii=False, indent=4) + '\n')
            
            print(job)
        current_date = datetime.now()
        formatted_date = current_date.strftime("%m_%d_%Y_%H_%M_%S")
        self.save_to_json(jobs_data, f'./jobs/{self.location}_{formatted_date}linkedin_jobs.json')  # Save to JSON
        self.save_to_csv(jobs_data, './jobs/linkedin_jobs.csv')
        self.teardown()
        return jobs_data
